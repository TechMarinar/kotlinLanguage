...
private fun FirCallableSymbol<*>.toSymbolForCall(
    ...
    val fakeOverrideOwnerLookupTag = when {
        ...
        // Unbound callable reference
        isReference -> {
            // TODO: remove StandardClassIds.Any comparison after fixing ValueClass::equals case
            runIf(fir.receiverTypeRef == null && containingClassLookupTag()?.classId != StandardClassIds.Any) {
                (explicitReceiver as? FirResolvedQualifier)?.toLookupTag()
            }
        }
        else -> null
