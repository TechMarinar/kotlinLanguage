...
class RenderIrElementVisitor(normalizeNames: Boolean = false, private val verboseErrorTypes: Boolean = true) :
    ...
    private class BoundSymbolReferenceRenderer(
        ...
        override fun visitClass(declaration: IrClass, data: Nothing?) = buildTrimEnd {
//            append(declaration.visibility)
//            append(' ')
//            append(declaration.modality.toString().toLowerCaseAsciiOnly())
//            append(' ')
//            append(
//                when (val kind = declaration.kind) {
//                    ClassKind.ENUM_ENTRY -> "enum entry"
//                    else -> kind.codeRepresentation ?: kind.toString().toLowerCaseAsciiOnly()
//                }
//            )
//            append(' ')
...
//            declaration.renderClassFlags()
//            renderDeclaredIn(declaration)
            append(renderClassWithRenderer(declaration, null, verboseErrorTypes))
        }

        override fun visitEnumEntry(declaration: IrEnumEntry, data: Nothing?) = buildTrimEnd {
//            append(declaration.parentAsClass.name.asString())
//            append(".")
//            append(declaration.name.asString())
//            append(' ')
//            renderDeclaredIn(declaration)
            append(renderEnumEntry(declaration))
        }
...
//            append(' ')
//            append(declaration.renderFieldFlags())
//            renderDeclaredIn(declaration)
            append(renderField(declaration, null, verboseErrorTypes))
        }
...
private fun StringBuilder.renderAsAnnotation(
    ...
    if (irAnnotation.typeArgumentsCount != 0) {
        (0 until irAnnotation.typeArgumentsCount).joinTo(this, ", ", "<", ">") { i ->
            irAnnotation.getTypeArgument(i)?.let { it.renderTypeWithRenderer(renderer, verboseErrorTypes) } ?: "null"
        }
    }
    ...
    (0 until irAnnotation.valueArgumentsCount).joinTo(this, separator = ", ", prefix = "(", postfix = ")") {
        "${valueParameterNames[it]} = ${StringBuilder().apply { renderAsAnnotationArgument(irAnnotation.getValueArgument(it), renderer, verboseErrorTypes) }}"
    }
}
