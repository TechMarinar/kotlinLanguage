...
internal class CacheStorage(val context: Context) {
    ...
    private fun saveCacheBitcodeDependencies() {
        val libraryToCache = context.config.cacheSupport.libraryToCache!!
        val usedBitcode = context.generationState.llvmImports.usedBitcode().groupBy { it.library }
        ...
        context.config.resolvedLibraries
                ...
                .forEach { library ->
                    ...
                    if (filesUsed == null && context.generationState.llvmImports.bitcodeIsUsed(library)
...
private fun determineCachesToLink(context: Context): CachesToLink {
    ...
    context.generationState.llvm.allCachedBitcodeDependencies.forEach { dependency ->
